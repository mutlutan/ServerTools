...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static bool IsInternetAvailable(string hostNameOrAddress = "8.8.8.8")
        {
            try
            {
                // 8.8.8.8 Google DNS, bağlantıyı kontrol etmek için
                using (Ping ping = new Ping())
                {
                    var reply = ping.Send(hostNameOrAddress, 5000); // 5 saniye timeout
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        Thread.Sleep(1000 * 30); //30sn bekle
